...
20 машинами (украинская регистрация) из вашего тестового списка.
...
> Примечание: в исходном списке в строке №7 была опечатка/артефакт
> распознавания текста — «Поlishчук». В скрипте это исправлено на
> «Полищук». Проверьте, пожалуйста, что это верное написание.
...
(`Trip.driver_id` + `Trip.vehicle_id`). Если вам нужна именно жёсткая
привязка "один водитель — одна машина", скажите — добавлю поле
`assigned_vehicle_id` в `Employee`.

## Дальнейшие шаги (когда будете готовы)
- аутентификация (JWT) для доступа к API
- Alembic-миграции вместо `create_all` (нужно при переходе с SQLite на PostgreSQL/MySQL)
- отчёты: пробег по водителю/машине за период, статистика по статусам рейсов
- уведомления (напр. диспетчеру, если рейс не стартовал вовремя)
